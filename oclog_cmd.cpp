...
*/
    ...
    ListPtr CreateERRecord(Machine& machine)
    {
        std::vector<ElementPtr> args;
        args.push_back(MakeString("execreport"));
        return CreateRecord(machine, args);
    }

    ElementPtr ParseExecutionReport(Machine& machine, StringPtr& line)
    {
        if (line->value.find("ExecutionReport") == std::string::npos)
            return MakeNone();
        FieldMap_t *pFields = machine.get_field_map("execreport");
        if (pFields == nullptr)
        {
            std::stringstream strm;
            strm << "execreport definition does not exist";
            throw std::runtime_error(strm.str());
        }
        ListPtr lst = CreateERRecord(machine);
        size_t pos = 0;
        for (auto& f : *pFields)
        {
            if (f.first == "raw")
    ...
    ElementPtr ParseExecutionReport(Machine& machine, std::vector<ElementPtr>& args)
    {
        ...
        if (!line)
            throw std::runtime_error("ParseExecutionReport expects a line");

        return ParseExecutionReport(machine, line);
    }
}
